import { collectUnclosedTwigControlTags, parseHybridDocument, type HtmlTagNode } from "@twig-plus/parser";
...
export function computeHtmlOnTypeEdit(source: string, offset: number): { start: number; end: number; newText: string } | null {
  ...
  if (collectUnclosedTwigControlTags(source, opening.start).includes("verbatim")) return null;
